    ...
    async function buynft(tokenId, price) {
        ...
        if (AmusementArcadeTokenContract === null) {
            ...
            await AmusementArcadeTokenContract.methods
                .approve(UtilsContract.options.address, utils.parseEther(price.toString()))
                                ...
                                .then(async (res) => {
                                    if (res.data.message === "ok") {
                                        const findIndex = await Selllists.findIndex((lists) => {
                                            if (parseInt(tokenId) === parseInt(lists.formInput.tokenid)) {
                                                return true;
                                            }
                                        });
                                        dispatch(
                                            updateMyLists({
                                                MyNFTlists: [...MyNFTlists, Selllists.splice(findIndex, 1)[0]],
                                            })
                                        );
        ...
        <div className="modal__wrapper">
            {Loading ? (
                <div className={Loading ? "parentDisable" : ""} width="100%" height="100%">
                    <div className="overlay-box">
                        <FadeLoader size={150} color={"#ffffff"} css={override} loading={Loading} z-index={"-9999"} text="Loading your content..." />
                    </div>
                </div>
            ) : (
                false
            ...
            <div className="single__modal">
                ...
                <div className="buy__nfts">
                    ...
                    <h5 className="text-center text-light">현재 작업은 토큰전송과 함께 가스비가 발생합니다. 진행하시겠습니까?</h5>
                    {/* 
            <div className="must__bid">
              <p>Sale ends</p>
            ...
            <div className="must__bid">
              ...
              <span className="money">{props.item.formInput.price} AAT</span>
                ...
                <button
                    ...
                    onClick={async () => {
                        console.log(props);
                        await buynft(props.item.formInput.tokenid, props.item.formInput.price);
                    }}
                >
                    Buy now
                </button>
