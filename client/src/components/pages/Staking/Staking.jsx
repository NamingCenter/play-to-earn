...
const Staking = () => {
    ...
    useEffect(async () => {
        await axios.post("http://15.165.17.43:5000/staking/rewards", { address: account }).then((res) => {
    ...
    return (
        <Container>
            {Loading ? (
                <div className={Loading ? "parentDisable" : ""} width="100%" height="100%">
                    <div className="overlay-box">
                        <FadeLoader size={150} color={"#ffffff"} css={override} loading={Loading} z-index={"1"} text="Loading your content..." />
                    </div>
                </div>
            ) : (
                false
            ...
            <Row>
                <Col sm="3">
                    ...
                    <div className="widget__form">
                        <Cards setLoading={setLoading} checkMyBalance={checkMyBalance} unclaimreward={unclaimreward} totalreward={reward} stakers={stakers} totalStakers={totalStakers} stakingAmount={stakingAmount} />
                    </div>
                </Col>
                <Col sm="8">
                                    ...
                                    onClick={async (e) => {
                                        if (StakingTokenContract !== null) {
                                            ...
                                            await AmusementArcadeTokenContract.methods
                                                .approve(StakingTokenContract._address, utils.parseUnits(stake, 18))
                                                ...
                                                .then(async () => {
                                                    await StakingTokenContract.methods
                                                        ...
                                                        .then(async (res) => {
                                                            console.log(res);
                                                            const stakerId = res.events.addStaker.returnValues.stakerId;
                                                            const address = res.events.addStaker.returnValues.stakerAddress;
                                                            const amount = parseInt(utils.formatEther(res.events.addStaker.returnValues.amount));
                                                            await axios
                                ...
                                <button
                                    ...
                                    onClick={async () => {
                                        if (StakingTokenContract !== null) {
                                            setLoading(true);
                                            await StakingTokenContract.methods
                                                .withdraw(utils.parseEther(unstake.toString()))
                                                .send({ from: account, gas: 3000000 })
                                                .then(async (res) => {
                                                    setLoading(true);
                                                    const stakerId = res.events.withdrawAmount.returnValues.stakerId;
                                                    const address = res.events.withdrawAmount.returnValues.stakerAddress;
                                                    const amount = parseInt(utils.formatEther(res.events.withdrawAmount.returnValues.amount));
                                                    await axios
                                                        .post("http://15.165.17.43:5000/staking", {
                                                            stakerId: stakerId,
                                                            address: address,
                                                        ...
                                                        .then(async (res) => {
                                                            const result = await StakingTokenContract.methods.stakers(stakerId).call();
                                                            setUnclaimreward(utils.formatEther(result.unclaimedRewards));
                                                            setStakingAmount(amount);
                                                            dispatch(
                                                                updateMyBalance({
