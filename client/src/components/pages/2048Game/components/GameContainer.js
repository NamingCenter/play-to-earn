...
export const GameContainer = ({ gameData, start, endGame, score, handleReset, setShowModal }) => {
    ...
    const getTransform = (elem) => {
        ...
        return [`transform: translate(${elemOffset[elem.oldIndex]?.left + 120}px, ${elemOffset[elem.oldIndex]?.top + 120}px)`, `transform: translate(${elemOffset[elem.id]?.left + 120}px, ${elemOffset[elem.id]?.top + 120}px)`];
    };
    ...
    return (
        <GameContainerStyle ref={childContainerRef}>
            {endGame && <Overlay handleReset={handleReset} score={score} setShowModal={setShowModal} />}
            {gameData.length > 0 &&
