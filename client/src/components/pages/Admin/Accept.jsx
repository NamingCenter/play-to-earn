...
const Accept = (props) => {
  ...
  async function checkApprove(address) {
    const result = await axios
      .post(`http://3.39.230.11:5000/game/getclaim`, { address: address })
  ...
  async function setClaim(address, amount) {
    if (TokenClaimContract !== null) {
      ...
      await TokenClaimContract.methods
        ...
        .then(() => {
          axios
            .post(`http://3.39.230.11:5000/game/setclaim`, {
              address: address,
              claim: true,
            })
            .then((res) => {
              ...
              onClick={async (e) => {
                if (rankingDB !== null && ranker[i] !== undefined) {
                  if ((await checkApprove(ranker[i].address)) === false) {
                    const snakeranker = await rankingDB.snakeranker.findIndex(
                      (data) => data.address === ranker[i].address
                    );
                    const tetrisranker = await rankingDB.tetrisranker.findIndex(
                      (data) => data.address === ranker[i].address
                    );
                    const puzzleranker = await rankingDB.puzzleranker.findIndex(
                      (data) => data.address === ranker[i].address
                    );
                    const mineranker = await rankingDB.mineranker.findIndex(
                      (data) => data.address === ranker[i].address
                    );
                    const result =
                      ((await snakeranker) !== -1
                        ? changeCost(snakeranker)
                        : 0) +
                      ((await tetrisranker) !== -1
                        ? changeCost(tetrisranker)
                        : 0) +
                      ((await puzzleranker) !== -1
                        ? changeCost(puzzleranker)
                        : 0) +
                      ((await mineranker) !== -1 ? changeCost(mineranker) : 0);
