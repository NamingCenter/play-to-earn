...
const Board = () => {
    ...
    useEffect(async () => {
        const snakeData = await axios.post(`http://15.165.17.43:5000/game/snakeScore`, { account: account });
    ...
    const sendPoint = async () => {
        ...
        const snakeData = await axios
            .post(`http://15.165.17.43:5000/game/snake`, {
        ...
        return (
            <React.Fragment>
                ...
                {sectionCard && (
                    <div id="snake_message">
                        <div id="snake_card">
                            ...
                            <h3 className="snake_card-value">{prevScore === undefined ? "None" : prevScore}</h3>
                            <h3 className="snake_card-heading">Score</h3>
                            <h3 className="snake_card-value">{score}</h3>
                            <div onClick={sendPoint}>점수 등록</div>
                            <div className="snake_restart button-space" onClick={handleRestart}></div>
                        </div>
                    </div>
                )}
                {/* Display container */}
                        ...
                        return (
                            <div key={rowIndex} className="snake_row">
                                {rowValue.map((cellValue, cellIndex) => {
                                    return <div key={cellIndex} className={`snake_cell ${cellValue === 1 ? "snake" : cellValue === 2 ? "food" : ""}`}></div>;
                                })}
                            </div>
                        );
                    })}
                ...
                <div id="snake_controller">
                    <div id="snake_ArrowLeft" className="snake_direction-button" onClick={() => setDirection("ArrowLeft")}></div>
                    <div id="snake_ArrowUp" className="snake_direction-button" onClick={() => setDirection("ArrowUp")}></div>
                    <div id="snake_ArrowDown" className="snake_direction-button" onClick={() => setDirection("ArrowDown")}></div>
                    <div id="snake_ArrowRight" className="snake_direction-button" onClick={() => setDirection("ArrowRight")}></div>
                </div>
                {/* Info */}
                <div id="snake_info">Controll movement by keyboard or virtual arrrows.</div>
            </React.Fragment>
        );
    }
};
